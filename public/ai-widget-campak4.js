/* ── detik AI Floating Widget (Campak v4 — resolved state) ── */
(function () {

  const RESOLVED_QUESTION = 'Kirana sudah vaksin kemarin. Masih perlu khawatir?';
  ...
  const css = `
    ...
    .ai-panel-msgs { flex: 1; overflow-y: auto; padding: 14px 14px 8px; display: flex; flex-direction: column; gap: 10px; background: #fafafa; min-height: 180px; }
    .ai-msg { max-width: 86%; font-size: 13px; line-height: 1.55; padding: 9px 12px; border-radius: 12px; animation: msgIn 0.18s ease both; }
    ...
    .ai-suggestions { display: flex; flex-direction: column; gap: 5px; padding: 0 14px 8px; background: #fafafa; }
    .ai-sug-btn { background: #fff; border: 1px solid #e8192c; color: #e8192c; border-radius: 20px; padding: 6px 12px; font-size: 12px; font-weight: 700; cursor: pointer; text-align: left; font-family: inherit; transition: background 0.15s, color 0.15s; }
    ...
    .ai-expand-link { text-align: center; padding: 8px; font-size: 11px; background: #fff; border-top: 1px solid #f0f0f0; flex-shrink: 0; }
    .ai-expand-link a { color: #e8192c; font-weight: 700; text-decoration: none; }
  ...
  fab.innerHTML = `
    <div id="detik-ai-panel">
      ...
      <div class="ai-suggestions" id="detik-ai-sugs">
        <button class="ai-sug-btn">✅ Kirana sudah vaksin kemarin. Masih perlu khawatir?</button>
      </div>
      <div class="ai-panel-footer">
        <input id="detik-ai-input" type="text" placeholder="Tanya tentang status vaksinasi..." autocomplete="off" />
  ...
  bubbleBtn.addEventListener('click', () => {
    ...
    if (panel.classList.contains('open') && msgs.children.length === 0) {
      addMsg('bot', 'Halo lagi! Kabar baik soal campak hari ini — ada yang ingin kamu tanyakan?');
    }
  ...
  function ask(text) {
    ...
    if (text.trim() === RESOLVED_QUESTION) {
      addMsg('bot', 'Mengarahkan ke chat lengkap...');
      sessionStorage.setItem('detik_ai_scenario', 'campak_resolved');
      sessionStorage.setItem('detik_ai_first_query', RESOLVED_QUESTION);
      setTimeout(() => { window.location.href = 'ai-chat-campak2.html'; }, 600);
    } else {
      addMsg('bot', 'Saat ini saya hanya dapat menjawab: <em>"Kirana sudah vaksin kemarin. Masih perlu khawatir?"</em>');
      setTimeout(() => { sugs.style.display = 'flex'; }, 800);
    }
  }

  sugs.querySelectorAll('.ai-sug-btn').forEach(btn => {
    btn.addEventListener('click', () => ask(RESOLVED_QUESTION));
  });
