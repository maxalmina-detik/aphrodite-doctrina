/* ── detik AI Floating Widget ── */
(function () {

  const STORAGE_KEY = 'detik_ai_mini_history';
  ...
  fab.innerHTML = `
    <div id="detik-ai-panel">
      ...
      <div class="ai-suggestions" id="detik-ai-sugs">
        <button class="ai-sug-btn">📰 Rangkum berita hari ini</button>
        <button class="ai-sug-btn">🔍 Jelaskan topik ini lebih lanjut</button>
        <button class="ai-sug-btn">📊 Apa dampak kebijakan ini?</button>
      </div>
      <div class="ai-panel-footer">
        <input id="detik-ai-input" type="text" placeholder="Tanya sesuatu..." autocomplete="off" />
      ...
      <div class="ai-expand-link">
        <a href="ai-chat.html">Buka Chat Penuh &rsaquo;</a>
      </div>
    </div>
  ...
  bubbleBtn.addEventListener('click', () => {
    ...
    if (panel.classList.contains('open') && msgs.children.length === 0) {
      addMsg('bot', 'Halo! Saya <strong>detik AI</strong> — asisten berita pintar Anda. Ada yang ingin Anda tanyakan tentang berita hari ini?');
    }
  ...
  async function ask(text) {
    ...
    try {
      const reply = await window.claude.complete({
        messages: [{
          role: 'user',
          content: `Kamu adalah detik AI, asisten berita dari detik.com. Jawab dalam bahasa Indonesia yang singkat, informatif, dan ramah. Maksimal 3 kalimat. Pertanyaan: ${text}`
        }]
      });
      typing.remove();
      addMsg('bot', reply);
      ...
      typing.remove();
      addMsg('bot', 'Maaf, saya tidak dapat menjawab saat ini. Silakan coba lagi.');
    }
  }

  // Suggestion buttons
  sugs.querySelectorAll('.ai-sug-btn').forEach(btn => {
    btn.addEventListener('click', () => ask(btn.textContent.replace(/^[^\w]+/, '').trim()));
  });
