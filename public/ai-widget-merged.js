...
(function () {
  ...
  const SCENARIOS = [
    {
      q: 'Siapa Tom Lembong dan kenapa ia jadi tersangka?',
      icon: '⚖️',
      scenario: 'tom_lembong',
      url: 'ai-chat-tom.html'
    },
    {
      q: 'Wabah campak 2026: seberapa luas penyebarannya?',
      icon: '🦠',
      scenario: 'campak_outbreak',
      url: 'ai-chat-campak.html'
    },
    {
      q: 'Campak ini berbahaya nggak?',
      icon: '💉',
      scenario: 'campak_bahaya',
      url: 'ai-chat-campak2.html'
    },
    {
      q: 'Masih bahaya nggak campaknya? Ada perkembangan baru?',
      icon: '🔄',
      scenario: 'campak_depth',
      url: 'ai-chat-campak2.html'
    },
    {
      q: 'Kirana sudah vaksin kemarin. Masih perlu khawatir?',
      icon: '✅',
      scenario: 'campak_resolved',
      url: 'ai-chat-campak2.html'
    }
  ];
  ...
  const sugBtnsHtml = SCENARIOS.map(s =>
    `<button class="ai-sug-btn" data-q="${s.q}">${s.icon} ${s.q}</button>`
  ).join('');
  ...
  fab.innerHTML = `
    <div id="detik-ai-panel">
      ...
      <div class="ai-suggestions" id="detik-ai-sugs">
        <div class="ai-sug-label">Pilih topik</div>
        ${sugBtnsHtml}
      </div>
  ...
  bubbleBtn.addEventListener('click', () => {
    ...
    if (panel.classList.contains('open') && msgs.children.length === 0) {
      addMsg('bot', 'Halo! Saya detik AI. Pilih topik di bawah atau ketik pertanyaanmu sendiri.');
    }
