/* ── detik AI Floating Widget (Campak v2) ── */
(function () {

  const CAMPAK_QUESTION = 'Campak ini berbahaya nggak?';
  ...
  fab.innerHTML = `
    <div id="detik-ai-panel">
      ...
      <div class="ai-suggestions" id="detik-ai-sugs">
        <button class="ai-sug-btn">💉 Campak ini berbahaya nggak?</button>
      </div>
      <div class="ai-panel-footer">
        <input id="detik-ai-input" type="text" placeholder="Tanya tentang campak..." autocomplete="off" />
  ...
  bubbleBtn.addEventListener('click', () => {
    ...
    if (panel.classList.contains('open') && msgs.children.length === 0) {
      addMsg('bot', 'Halo! Saya <strong>detik AI</strong> — ada yang ingin Anda tanyakan tentang wabah campak 2026?');
    }
  ...
  function ask(text) {
    ...
    if (text.trim() === CAMPAK_QUESTION) {
      addMsg('bot', 'Mengarahkan ke chat lengkap...');
      sessionStorage.setItem('detik_ai_scenario', 'campak_bahaya');
      sessionStorage.setItem('detik_ai_first_query', CAMPAK_QUESTION);
      setTimeout(() => { window.location.href = 'ai-chat-campak2.html'; }, 600);
    } else {
      addMsg('bot', 'Saat ini saya hanya dapat menjawab: <em>"Campak ini berbahaya nggak?"</em>');
  ...
  sugs.querySelectorAll('.ai-sug-btn').forEach(btn => {
    btn.addEventListener('click', () => ask(CAMPAK_QUESTION));
  });
